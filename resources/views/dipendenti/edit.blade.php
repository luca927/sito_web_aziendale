...
<div class="py-6 max-w-4xl">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <form method="POST" action="{{ route('dipendenti.update', $dipendente) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="nome"
                                   value="{{ old('nome', $dipendente->nome) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="cognome"
                                   value="{{ old('cognome', $dipendente->cognome) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="dataDiNascita"
                                   value="{{ old('dataDiNascita', $dipendente->dataDiNascita) }}"
                        ...
                        <div>
                            ...
                            <select name="sesso"
                                ...
                                <option value="M"     {{ old('sesso', $dipendente->sesso) === 'M'     ? 'selected' : '' }}>Maschile</option>
                                <option value="F"     {{ old('sesso', $dipendente->sesso) === 'F'     ? 'selected' : '' }}>Femminile</option>
                                <option value="Altro" {{ old('sesso', $dipendente->sesso) === 'Altro' ? 'selected' : '' }}>Altro</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Stato Civile</label>
                            <select name="stato_civile"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                <option value="">— Seleziona —</option>
                                <option value="celibe/nubile"  {{ old('stato_civile', $dipendente->stato_civile) === 'celibe/nubile'  ? 'selected' : '' }}>Celibe / Nubile</option>
                                <option value="coniugato/a"    {{ old('stato_civile', $dipendente->stato_civile) === 'coniugato/a'    ? 'selected' : '' }}>Coniugato/a</option>
                                <option value="divorziato/a"   {{ old('stato_civile', $dipendente->stato_civile) === 'divorziato/a'   ? 'selected' : '' }}>Divorziato/a</option>
                                <option value="vedovo/a"       {{ old('stato_civile', $dipendente->stato_civile) === 'vedovo/a'       ? 'selected' : '' }}>Vedovo/a</option>
                                <option value="separato/a"     {{ old('stato_civile', $dipendente->stato_civile) === 'separato/a'     ? 'selected' : '' }}>Separato/a</option>
                            </select>
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Indirizzo di Residenza</label>
                            <input type="text" name="indirizzoResidenza"
                                   value="{{ old('indirizzoResidenza', $dipendente->indirizzoResidenza) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="telefono"
                                   value="{{ old('telefono', $dipendente->telefono) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <div>
                            ...
                            <input type="email" name="recapitieMail"
                                   value="{{ old('recapitieMail', $dipendente->recapitieMail) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('recapitieMail') border-red-400 @enderror"
                                   placeholder="mario.rossi@azienda.it">
                            @error('recapitieMail') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Indirizzo</label>
                            <input type="text" name="indirizzo"
                                   value="{{ old('indirizzo', $dipendente->indirizzo) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <div>
                            ...
                            <input type="text" name="lng"
                                   value="{{ old('lng', $dipendente->lng) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Es. 9.1900">
                        </div>

                        <div class="md:col-span-2 border-t pt-4 mt-2">
                            <h3 class="text-sm font-semibold text-gray-600 mb-3">🔐 Aggiorna Password <span class="font-normal text-gray-400">(lascia vuoto per non modificarla)</span></h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="codice_fiscale"
                                   value="{{ old('codice_fiscale', $dipendente->codice_fiscale) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <div>
                            ...
                            <input type="text" name="iban"
                                   value="{{ old('iban', $dipendente->iban) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="IT60 X054 2811 1010 0000 0123 456">
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Dati Bancari (note aggiuntive)</label>
                            <input type="text" name="DatiBancari"
                                   value="{{ old('DatiBancari', $dipendente->DatiBancari) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Es. Banca, filiale, intestatario">
                        </div>
                ...
                <div id="tab-formazione" class="tab-panel hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Livello Esperienza</label>
                            <select name="livello_esperienza"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                <option value=""> Seleziona </option>
                                <option value="1" {{ old('livello_esperienza', $dipendente->livello_esperienza) == 1 ? 'selected' : '' }}>🟢 Junior</option>
                                <option value="2" {{ old('livello_esperienza', $dipendente->livello_esperienza) == 2 ? 'selected' : '' }}>🔵 Middle</option>
                                <option value="3" {{ old('livello_esperienza', $dipendente->livello_esperienza) == 3 ? 'selected' : '' }}>🟠 Senior</option>
                                <option value="4" {{ old('livello_esperienza', $dipendente->livello_esperienza) == 4 ? 'selected' : '' }}>🔴 Expert</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Corsi e Formazione</label>
                            <input type="text" name="Corsi_e_Formazione"
                                   value="{{ old('Corsi_e_Formazione', $dipendente->Corsi_e_Formazione) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Es. Corso sicurezza, Excel avanzato...">
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Competenze Tecniche</label>
                            <textarea name="Competenze" rows="3"
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                                      placeholder="Elenca le competenze tecniche principali...">{{ old('Competenze', $dipendente->Competenze) }}</textarea>
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Esperienze Professionali</label>
                            <textarea name="Esperienze" rows="4"
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                                      placeholder="Descrivi le esperienze lavorative precedenti...">{{ old('Esperienze', $dipendente->Esperienze) }}</textarea>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="mansione"
                                   value="{{ old('mansione', $dipendente->mansione) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Es. Elettricista, Magazziniere...">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Patenti</label>
                            <input type="text" name="patenti"
                                   value="{{ old('patenti', $dipendente->patenti) }}"
                                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="Es. B, C, CQC, Muletto...">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="note" rows="3"
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                                      placeholder="Eventuali note o informazioni aggiuntive...">{{ old('note', $dipendente->note) }}</textarea>
                        </div>
...
<script>
    window.__errorFields = <?php echo json_encode($errors->keys()); ?>;
</script>
...
// Apri il tab che contiene eventuali errori di validazione
const errorFields = window.__errorFields || [];

if (errorFields.length > 0) {
    const tabFields = {
        anagrafica:   ['nome', 'cognome', 'dataDiNascita', 'data_assunzione', 'sesso', 'stato_civile', 'indirizzoResidenza'],
        contatti:     ['telefono', 'recapitieMail', 'indirizzo', 'lat', 'lng', 'password'],
        documenti:    ['codice_fiscale', 'iban', 'DatiBancari', 'documentiIdentita', 'tipo_documento', 'numero_documento', 'scadenza_documento'],
        formazione:   ['livello_esperienza', 'Corsi_e_Formazione', 'Competenze', 'Esperienze'],
        assegnazioni: ['mansione', 'patenti', 'note'],
    };
    outer: for (const [tab, fields] of Object.entries(tabFields)) {
        for (const f of errorFields) {
            if (fields.includes(f)) { switchTab(tab); break outer; }
        }
    }
} else {
    renderTab();
